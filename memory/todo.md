...
- [ ] [P1] 完成下板 `DecisionAO` 的单一写入所有权迁移。
  - 范围：`hero_down/qp/decision_ao.qm`、生成的 `decision_ao.c/.h` 与 `Tasks/Src/task_state.c`。
...
- [ ] [P2] 对下板 `chassisControl` 做行为等价的函数拆分与控制逻辑整理。
  ...
  - 状态：2026-07-28 至 2026-07-30 已完成首轮函数拆分、初始化收口、周期计数器命名和无效功率估算参数清理；模块私有 `chassis_runtime_t`、`gimbal_runtime_t`、`shoot_runtime_t` 均已内嵌完整控制主体并收纳全部跨周期状态，原独立可写控制实例已删除。三个主要控制实现已移除万能头并使用私有小写叶子字段宏，通用遥控叶子宏集中到 `general_define.h`；鼠标滤波状态迁回 gimbal，Task/模块跨边界写入改为 API 或只读指针，B2B 姿态、云台目标和上位机发送改为局部快照，并删除 shoot 与 music 中仅定义未使用的一批僵尸变量。全部 Task 源文件已改为精确 include，三个控制实现已统一为 `if(condition){` 同行大括号风格。已确认 `auto_slope` 自统一仓库首个提交起没有置位入口，现已删除其飞坡加速、限速放宽和退出过渡死代码；关节 `CLIMB/OUTCLIMB` 与台阶模式保留。其余控制参数与执行顺序未调整。
  - 验证：此前受影响的 chassis、gimbal、stir、joint 及直接相关 Task 源文件已使用 ARMClang 6.22.0 完成语法编译；私有宏引用扫描无僵尸宏，通用小写输入宏未发现跨模块名称冲突，Keil 工程 XML 可解析且三个 internal 头及 `initial_task.h` 均已收录。本次 `auto_slope` 清理已通过 `git diff --check` 和全仓残留扫描，ARMClang 因本机许可证不可用未完成本次语法编译。Keil 全量构建仍未完成。仍需完成上下限/零输入/模式切换静态用例，并在实机核对跟随、自旋、台阶、掉轮降级、功率限制和急停。
