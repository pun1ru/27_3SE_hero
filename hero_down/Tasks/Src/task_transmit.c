...
//#define DEBUG_FRAME_UP
//#define DEBUG_FRAME_CTRL
//#define DEBUG_FRAME_SYSID
...
void MotorControlCANSend(void){
    ...
    if((pDecisionAO->ctrl_terminal == CONTROL_STOP) || (pDecisionAO->can_enable == CAN_DISABLE)){
        
        
        if((slot & 1) == 0){
            DM_MITControl_JointsSendTorq(&hfdcan1, (float[4]){0,0,0,0});
            CANTransmit_I16(&hfdcan2, CAN2_CHASSIS_TX, 0,0,0,0);
        }
        ...
        if(slot % 5 == 3){
            CANTransmit_I16(&hfdcan2, SUPERCAP_TX, 0,0,
        ...
        if((slot & 1) == 0){
                        ...
                        CANTransmit_I16(&hfdcan2, CAN2_CHASSIS_TX, 0,0,0,0);
            #else
                        CANTransmit_I16(&hfdcan2, CAN2_CHASSIS_TX,
