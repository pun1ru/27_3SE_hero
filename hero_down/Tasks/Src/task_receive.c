...
DMMotorRec jointMotorEec[4];
const DMMotorRec* _jointMotorEec = jointMotorEec;

DMMotorRec stirMotorRec;
const DMMotorRec* _stirMotorRec = &stirMotorRec;
...
DMMotorRec caterpillarMotorRec[2];
const DMMotorRec* _caterpillarMotorRec = caterpillarMotorRec;
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs)
{
    ...
    if ((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != RESET)
    {
        ...
        while (HAL_FDCAN_GetRxFifoFillLevel(hfdcan, FDCAN_RX_FIFO0) > 0U)
        {
            ...
            case CAN1_JOINT_RX_FIRST:
            case CAN1_JOINT_RF_RX:
            case CAN1_JOINT_RB_RX:
            case CAN1_JOINT_RX_LAST:
            {
                uint8_t idx = RxHeader.Identifier - CAN1_JOINT_RX_FIRST;
...
void HAL_FDCAN_RxFifo1Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs)
{
    if ((RxFifo0ITs & FDCAN_IT_RX_FIFO1_NEW_MESSAGE) != RESET)
    {
        ...
        while (HAL_FDCAN_GetRxFifoFillLevel(hfdcan, FDCAN_RX_FIFO1) > 0U)
        {
            ...
            if (hfdcan == &hfdcan2) {
                switch (RxHeader.Identifier) {
                case CAN2_CHASSIS_RX_FIRST:
                case CAN2_CHASSIS_RF_RX:
                case CAN2_CHASSIS_RB_RX:
                case CAN2_CHASSIS_RX_LAST:
                {
                    uint8_t idx = RxHeader.Identifier - CAN2_CHASSIS_RX_FIRST;
                ...
                case CAN2_CATERPILLAR_RX_FIRST:
                case CAN2_CATERPILLAR_RX_LAST:
                {
                    uint8_t idx = RxHeader.Identifier - CAN2_CATERPILLAR_RX_FIRST;
...
void UpperCommRecHandler(const uint8_t* rec_buf, uint32_t size){
    if((rec_buf == NULL) || (size != UPPER_PC_COMM_REC_LENGTH)){
        return;
    }

    if((rec_buf[0] != UPPER_PC_COMM_REC_SOF)
       || (rec_buf[size - 1U] != UPPER_PC_COMM_REC_EOF)){
        return;
    }

    memcpy(&upperComputerComm.Receive, rec_buf, sizeof(upperComputerComm.Receive));
    upperComputerComm.Receive.target_pitch_angle_d =
        DoubleEdgeLimiter(upperComputerComm.Receive.target_pitch_angle_d, 0.0f, 30.0f);
    upperComputerComm.Receive.target_yaw_angle_d =
        DoubleEdgeLimiter(upperComputerComm.Receive.target_yaw_angle_d, -10.0f, 10.0f);
