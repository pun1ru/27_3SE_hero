...
{
	if((start_x+radius)>1920||(int)(start_x-radius)<0\
		||(start_y+radius)>1080||(int)(start_y-radius)<0)
		return CoordinateError;
	if(layer>LAYER_NUMBER)
		return LayerError;
	if(color>COLOR_NUMBER)
...
{
	if((start_x+halfshaft_x)>1920||(int)(start_x-halfshaft_x)<0\
		||(start_y+halfshaft_y)>1080||(int)(start_y-halfshaft_y)<0)
		return CoordinateError;
	if(layer>LAYER_NUMBER)
		return LayerError;
	if(color>COLOR_NUMBER)
...
{
	if(start_x<halfshaft_x||start_x>1920-halfshaft_x\
		||start_y<halfshaft_y||start_y>1080-halfshaft_y)
...
void UIframeTransmit(uint8_t *data_UIframe,uint16_t SubcontentID)
{
	...
	static uint8_t transmitdata[128];
	memcpy(transmitdata,&RefereeFrame,sizeof(RefereeFrame_t));
    Append_CRC_Check_Sum(transmitdata, transmitdata[1]);
	/* 等待上次DMA发送完成，避免帧碰撞丢包 */
	/* 注：REFEREE_UART始终处于RX DMA模式，TX期间状态为BUSY_TX_RX而非BUSY_TX */
	{
		uint32_t dma_timeout = 100000U;
		HAL_UART_StateTypeDef uart_state;
		do
		{
			uart_state = HAL_UART_GetState(&REFEREE_UART);
		} while ((uart_state == HAL_UART_STATE_BUSY_TX ||
		          uart_state == HAL_UART_STATE_BUSY_TX_RX) && --dma_timeout);

		if (dma_timeout == 0U)
		{
			/* 超时：上一帧DMA可能卡死，跳过本次发送避免缓冲区冲突 */
			return;
		}
	}
