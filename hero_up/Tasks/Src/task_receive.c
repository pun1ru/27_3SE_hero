...
const NormRemoteCmd *const _normRemoteCmd = &normRemoteCmd;

/**
 * @brief 遥操作指令接收任务，处理接收遥控器数据，自定义控制器等
 *新增超时时间
 */
static const TickType_t remote_timeout_ticks = pdMS_TO_TICKS(500U);
...
const DJIGMotorRec *const _fricMotorRec = fricMotorRec;

DJIGMotorRec pitchMotorRec;
const DJIGMotorRec *const _pitchMotorRec = &pitchMotorRec;

/* yaw DM 电机（从下板搬迁至 CAN3） */
DMJ4310MotorRec DMyawMotorRec;
const DMJ4310MotorRec *const _DMyawMotorRec = &DMyawMotorRec;
/**
...
/*云台姿态记录*/
Pose gimbalPose;
const Pose *const _gimbalPose = &gimbalPose;
...
#define IMU_TEMPERATURE_KP 100.0f
#define IMU_TEMPERATURE_KI 100.0f
#define IMU_TEMPERATURE_KD 10.0f
#define IMU_HEATER_MAX_OUTPUT 500.0f
static void OnboardIMUTemperatureControl(float real_temperature){
    static float previous_error;
    static float previous_previous_error;
    float error = OUT_STANDING_TEMPERATURE - real_temperature;
    float output = IMU_TEMPERATURE_KP * error + IMU_TEMPERATURE_KI * (error + previous_error + previous_previous_error) + IMU_TEMPERATURE_KD * (error - previous_error);

    previous_previous_error = previous_error;
    previous_error = error;
    if(output > IMU_HEATER_MAX_OUTPUT){
        output = IMU_HEATER_MAX_OUTPUT;
    }
    if(output < 0.0f){
        output = 0.0f;
    }
    htim3.Instance->CCR4 = (uint16_t)output;
}
...
/* 偏置已由 GetIMUOffset 写入实测值，直接进入 EKF 解算。 */
#if defined ONBOARD_EKF_SOLVE
...
const UpperComputerComm *const _upperComputerComm = &upperComputerComm;
/**
...
void UpperPCCommTask(void *argument){
    ...
    while(1){
        ...
        upperComputerComm.Send.cam_target = pDecisionAO->cam_target;
...
// HAL_UART_Transmit_DMA(&MINIPC_UART, (uint8_t*)&(upperComputerComm.Send), UPPER_PC_COMM_SEND_LENGTH);
        ...
        /* B2B CAN: GimbalTarget pitch 100Hz -> hero_down (yaw 由上板直接控制) */
        {
            ...
            if(_worldGimbal->enable){
                ty = _worldGimbal->WorldGimbalControl.q_yaw_cmd_deg;
                tp = _worldGimbal->WorldGimbalControl.q_pitch_cmd_deg;
            }
            B2BSendGimbalTarget(ty, tp);
        }
        // memset(upperComputerComm.Send.reserved, 0, 3);  // 发送后清零，由task_state下一周期置位
        current_tick_count = xTaskGetTickCount();
...
static DataFromJudge bulletSpeed;
// float predict_speed_array[40]={0};
// uint8_t speedindex=0;
/// @brief 弹速统计处理
/// @param shooter_30_state
static void BulletSpeedStatistics(uint8_t shooter_30_state){
    ...
    for(int i = 0; i < count; i++){
        ...
        if(fabsf(speed[i] - median) <= 0.01f){
            sum2++;
            if(fabsf(speed[i] - median) < 0.001f){
                sum3++; // 严格相等
            }
        }
        ...
        if(fabsf(speed[i] - average) <= 0.01f){
            sum4++;
            if(fabsf(speed[i] - average) < 0.001f){
                sum5++;
            }
        }
    }
    ...
    bulletSpeed.predict_speed =
        (max_hit_rate == bulletSpeed.hit_rate_median) ? median : (max_hit_rate == bulletSpeed.hit_rate_mode) ? mode
                                                                                                             : average;
...
/*onboard imu init*/
    ...
    HAL_UARTEx_ReceiveToIdle_DMA(&LASER_UART,
                                 uart10RecBuffer,
                                 sizeof(uart10RecBuffer));
