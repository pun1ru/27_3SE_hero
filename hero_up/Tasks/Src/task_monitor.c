...
#define LED_BREATH_HALF_CYCLES 100U
#define LED_BREATH_FULL_CYCLES (2U * LED_BREATH_HALF_CYCLES)
...
static void task_monitor_check_periodic_task(TaskMonitorId task_id,
    ...
    period_disturbed = task_period_ticks(task_id) > (expected_period_ticks[task_id] + TASK_MONITOR_PERIOD_TOLERANCE_TICKS);
...
static void task_monitor_update_led(void){
    static uint8_t breath_count;
    uint8_t brightness;
    int led_index;

    breath_count++;
    if(breath_count >= LED_BREATH_FULL_CYCLES){
        breath_count = 0U;
    }

    if(breath_count < LED_BREATH_HALF_CYCLES){
        brightness = breath_count;
    } else {
        brightness = (uint8_t)(LED_BREATH_FULL_CYCLES - breath_count);
    }

    for(led_index = 0; led_index < WS2812_NUM_LEDS; led_index++){
        WS2812_SetLED(led_index, 0U, (uint8_t)(brightness >> 1U), 0U);
    }
    WS2812_Send();
}

static void music_play_score(const float *score, int note_count,
...
static void music_check_circuit_error(void){
    ...
    pitch_error = pitch_frame_count == _pitchMotorRec->frame_counter;
    yaw_error = yaw_frame_count == _DMyawMotorRec->frame_counter;

    for(motor_index = 0; motor_index < 6; motor_index++){
        fric_error[motor_index] = fric_frame_count[motor_index] == _fricMotorRec[motor_index].frame_counter;
    }

    pitch_frame_count = _pitchMotorRec->frame_counter;
    yaw_frame_count = _DMyawMotorRec->frame_counter;
    for(motor_index = 0; motor_index < 6; motor_index++){
        fric_frame_count[motor_index] = _fricMotorRec[motor_index].frame_counter;
    }
}
...
void TaskMonitorMarkControlExit(uint32_t cycle_count){
    ...
    timer->imu_to_est_us = task_monitor_cycles_to_us(timer->cyc_est_entry - timer->cyc_imu_notify);
    timer->est_exec_us = task_monitor_cycles_to_us(timer->cyc_est_exit - timer->cyc_est_entry);
    timer->est_to_ctrl_us = task_monitor_cycles_to_us(timer->cyc_ctrl_entry - timer->cyc_est_exit);
    timer->ctrl_exec_us = task_monitor_cycles_to_us(timer->cyc_ctrl_exit - timer->cyc_ctrl_entry);
    timer->chain_total_us = task_monitor_cycles_to_us(timer->cyc_ctrl_exit - timer->cyc_imu_notify);
    ...
    while(1){
        ...
        if(xQueueReceive(g_musicQueue, &music_receive, 0U) == pdPASS){
            switch(music_receive){
                ...
                case 2:
                case 3:
                    music_play_score(battlefiled,
                                     (int)(sizeof(battlefiled) / sizeof(battlefiled[0])),
                                     150,
                                     -8.0f);
                    break;

                case 4:
                    play_music_notes(Kyokorigyu,
                                     (int)(sizeof(Kyokorigyu) / sizeof(Kyokorigyu[0])),
                                     BUZZER_TIM);
                    break;
