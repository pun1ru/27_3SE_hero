...
/**
 * @brief   解析键位和开关到 NormRemoteCmd
 */
static void b2bParseKeysSwitch(uint8_t *data, NormRemoteCmd *dst){
    uint8_t sb = data[2];
    ...
    switch(can_id){
        ...
        case B2B_DOWN_GIMBAL_INPUT:
            /* yaw_cmd + pitch_cmd 一起发来 */
            {
                ...
                if(!isfinite(yaw_cmd) || !isfinite(pitch_cmd) || fabsf(yaw_cmd) > 180.0f || fabsf(pitch_cmd) > 180.0f){
        ...
        case B2B_DOWN_KEYS_SWITCH:
            b2bParseKeysSwitch(data, &normRemoteCmd);
            return 1U;
